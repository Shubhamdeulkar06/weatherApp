...
const getData = async (event) => {
  ...
  const fetchData = await fetch(
    `http://api.weatherapi.com/v1/forecast.json?key=3b6fa2ae2fef43e3bb5163813231102&q=${city}&days=6`
  );
  ...
  const futureForcast = () => {
    for (let i = 0; i < 5; i++) {
      ...
      nextSunrise[i].innerHTML = data.forecast.forecastday[i + 1].astro.sunrise;
